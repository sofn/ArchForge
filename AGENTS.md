...
4. **Verify each step** — per the per-edit verification protocol below
...
Tool-specific configs are thin pointers only; never maintain two copies.
AI-facing files are written in **English**.
...
- Place agent-loop related files inside the owning `.agents/changes/<name>/`
  directory (or a sibling `.agents/` location), never in a top-level
  `.agent-loop/`.
...
- Canonical backend standard:
  `.agents/skills/archforge-project-standard/standard.md`
