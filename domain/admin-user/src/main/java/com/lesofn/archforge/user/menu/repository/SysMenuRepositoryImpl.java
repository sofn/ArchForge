...
import com.lesofn.archforge.user.domain.QSysMenu;
import com.lesofn.archforge.user.domain.QSysRoleMenu;
import com.lesofn.archforge.user.domain.QSysUser;
...
/** SysMenuRepository实现类，同时支持Spring Data JPA和QueryDSL 上层只需要使用这个类即可 */
@Repository
...
public class SysMenuRepositoryImpl implements SysMenuRepositoryCustom {
    ...
    public List<SysMenu> selectMenuListByUserId(Long userId) {
        ...
        return queryFactory
                ...
                .from(menu)
                .leftJoin(roleMenu)
                .on(menu.menuId.eq(roleMenu.menuId))
                .leftJoin(user)
                .on(roleMenu.roleId.eq(user.roleId))
                .where(user.userId.eq(userId).and(menu.status.eq(1)).and(menu.deleted.eq(false)))
