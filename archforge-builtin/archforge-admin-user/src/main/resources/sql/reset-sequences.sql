...
SELECT setval('cms_category_id_seq', COALESCE((SELECT MAX(id) FROM cms_category), 1));
SELECT setval('cms_article_id_seq', COALESCE((SELECT MAX(id) FROM cms_article), 1));
