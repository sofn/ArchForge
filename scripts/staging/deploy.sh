...
for i in $(seq 1 30); do
    if docker compose -f docker-compose.staging.yml exec -T postgres pg_isready -U "${DB_USERNAME:-archforge}" >/dev/null 2>&1; then
        break
    fi
    sleep 1
done
docker compose -f docker-compose.staging.yml exec -T postgres psql -U "${DB_USERNAME:-archforge}" -d postgres -f /docker-entrypoint-initdb.d/init-db.sql
...
for i in $(seq 1 120); do
    if docker compose -f docker-compose.staging.yml logs backend 2>/dev/null | grep -q 'Started .* in '; then
        break
    fi
    sleep 1
done
...
docker cp "${REPO_ROOT}/domain/admin-user/src/main/resources/sql" "$(docker compose -f docker-compose.staging.yml ps -q postgres):/tmp/seed"
...
echo ""
echo "Staging deployed. Frontend: http://localhost"
