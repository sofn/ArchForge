 ...
 *
 * <p>
 * Empty values (null, blank string, empty collection, empty array) are skipped — non-blank
...
public final class QueryHelp {
    ...
    private static final ClassValue<List<FieldMeta>> FIELD_CACHE = new ClassValue<>() {
        @Override
    ...
    public static <R, Q> Predicate getPredicate(
        ...
        try {
            for (FieldMeta fm : FIELD_CACHE.get(criteria.getClass())) {
                ...
                @Nullable
                Join<R, ?> join = q.joinName().isEmpty() ? null : resolveJoin(root, joinCache, q);
    ...
    private static <R> Predicate buildPredicate(
        ...
        return switch (type) {
            ...
            case LESS_THAN_OR_EQUAL -> cb.lessThanOrEqualTo(
                    path.as((Class<Comparable>) fieldType), (Comparable) value);
            // Fix 5: strict >
            case GREATER_THAN -> cb.greaterThan(path.as((Class<Comparable>) fieldType), (Comparable) value);
            // Fix 5: strict <
            case LESS_THAN -> cb.lessThan(path.as((Class<Comparable>) fieldType), (Comparable) value);
            ...
            case BETWEEN -> {
                ...
                if (bounds.size() != 2) {
                    throw new IllegalArgumentException("BETWEEN requires exactly 2 elements, got " + bounds.size() +
